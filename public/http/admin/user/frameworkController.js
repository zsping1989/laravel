define(['app',dataPath(),'joint','admin/public/headerController','admin/public/leftController'], function (app,datas,joint) {
    ...
    app.register.controller('admin-user-frameworkCtrl', ["$scope",'$rootScope', 'Model','View','$alert', function ($scope,$rootScope,Model,View,$alert) {
        //数据缓存,用于方便更新数据
        var maindata = window.cacheData['admin-user-index'] || datas.list;
        window.cacheData['admin-user-index'] = maindata;
        $scope.data_key = 'admin-user-index';
        ...
        var bart = member(300, 70, '超级管理员', 'BartSimpso', 'male.png', '#30d0c6');

        var homer = member(90, 200, ' Marketing', 'HomerSimpson', 'male.png', '#7c68fd', '#f1f1f1');
        var marge = member(300, 200, 'VPSales', 'MargeSimpson', 'female.png', '#7c68fd', '#f1f1f1');
        var lisa = member(500, 200, 'VPProduction', 'LisaSimpson', 'female.png', '#7c68fd', '#f1f1f1');

        var maggie = member(400, 350, 'Manager', 'MaggieSimpson', 'female.png', '#feb563');
        var lenny = member(190, 350, 'Manager', 'LennyLeonard', 'male.png', '#feb563');
        var carl = member(190, 500, 'Manager', 'CarlCarlson', 'male.png', '#feb563');
